...
const mapBookFromApi = (raw: BookApiShape): Book => {
  ...
  const publishedYear = toNumber(raw.publishedYear ?? raw.publicationYear ?? raw.year ?? raw.published_date);
  ...
  const loadBook = async (bookId: string) => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const foundRaw = (data as BookApiShape[]).find((b) => String(b.bookId ?? b.id ?? '') === String(bookId));
  ...
  return (
    <div className="min-h-screen py-12 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="glass-effect rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8 md:p-12">
            ...
            <div className="md:col-span-2">
              <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3 leading-tight">{book.title}</h1>
        ...
        <Modal isOpen={isListModalOpen} onClose={() => setIsListModalOpen(false)} title="Select a Reading List">
              ...
              onClick={async () => {
                ...
                try {
                  await createReviewApi({
                    bookId: id,
                    userId: user.id,                          // ✅ EKLENDİ (zorunlu)
                    userName: user.name ?? 'User',            // ✅ EKLENDİ (UI’da isim için)
                    rating: safeRating,
        ...
        <div className="mt-8 glass-effect rounded-3xl shadow-xl border border-white/20 p-8 md:p-12">
          ...
          {isReviewsLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-600 text-lg">No reviews yet. Be the first to share your thoughts!</p>
            </div>
          ) : (
            <div className="space-y-4">
              {reviews.map((r) => (
                <div key={r.id} className="rounded-2xl border border-slate-200 p-5 bg-white/60">
                  <div className="flex items-start justify-between">
                    <div className="text-slate-900 font-medium text-base">{r.userName?.trim() ? r.userName : 'User'}</div>
                  ...
                  <p className="mt-3 text-slate-700 leading-relaxed">{r.comment?.trim() ? r.comment : '-'}</p>
